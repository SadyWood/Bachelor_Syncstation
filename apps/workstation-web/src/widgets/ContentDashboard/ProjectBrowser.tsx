...
export default function ProjectBrowser({ title, onClose }: Omit<WidgetProps, 'id'>) {
  ...
  const toggleProject = useCallback(
    async (project: ProjectSummaryType) => {
  ...
  useEffect(
    () =>
      addTypedEventListener<SelectNodeByIdEvent>(EVENT_NAMES.SELECT_NODE_BY_ID, async (e) => {
          ...
          {projects.map((project) => {
            ...
            return (
              <div key={project.nodeId} className="mb-2">
                <div
                  className="flex items-center gap-2 px-2 py-2 text-sm font-medium cursor-pointer rounded hover:bg-gray-100"
                ...
                {isExpanded && (
                  <div className="mt-1">
                    {tree.length === 0 ? (
                      <div className="px-4 py-2 text-xs ws-muted">No content</div>
                    ) : (
