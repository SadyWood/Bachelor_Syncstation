...
**A comprehensive guide for students working on HK26 Syncstation**
...
In HK26 Syncstation, we use **type safety** all the way from database to frontend. This means TypeScript can tell us if we're trying to use data incorrectly, BEFORE we run the code.
...
**Important:** HK26 Syncstation uses **UUIDv7** for IDs, not auto-increment integers.
...
const logs = await db.query.logEntries.findMany({
  orderBy: (logEntries, { desc }) => [desc(logEntries.id)], // ✅ Newest first!
});
```
...
**Auto Increment:**
```typescript
const logId = 12345; // Easy to guess the next log is 12346
const apiUrl = `/api/log-entries/${logId}`; // Reveals you have ~12,345 logs
```

**UUIDv7:**
```typescript
const logId = '018e5c7a-8c3e-7890-1234-56789abcdef0'; // Impossible to guess
const apiUrl = `/api/log-entries/${logId}`; // Reveals nothing about your data
```
...
```typescript
// When you insert a log entry, ID is generated automatically
...
console.log(newLog.id); // "018e5c7a-8c3e-7890-1234-56789abcdef0"
```
...
3. API ROUTE (Backend)
   ...
   fastify.get('/syncstation/log-entries', async (request, reply) => {
     const dbLogs = await dbSync.query.logEntries.findMany(...);

     // Validate + transform
     const logs = dbLogs.map(log => LogEntrySchema.parse(log));
...
4. FRONTEND (React Native + Expo)
   ...
   const { data } = useQuery({
     ...
     queryFn: async () => {
       const response = await apiClient.get(`/syncstation/log-entries?nodeId=${nodeId}`);
       return response.data.items as LogEntry[];
     }
   });

   // data is now typed as LogEntry[] ✅
   // TypeScript knows that data[0].title exists!
...
│   ├── platforms.ts
│   └── invites.ts
├── workstation/           ← Content data (read-only for Syncstation)
│   ├── content.ts        ← Content nodes (projects, episodes)
│   ├── tenants.ts
│   ├── media.ts
│   └── ...
└── syncstation/           ← YOUR database (log entries, attachments)
    ├── schema.ts
    └── ...
```
...
HK26 Syncstation has three separate databases:

1. **users** - User authentication (shared between all apps)
2. **workstation** - Content data (you READ from this to link logs to content nodes)
3. **syncstation** - Your log entries and attachments (you OWN this)

**Why?** This separation allows each team to work independently while sharing common data. It also helps with:
...
```typescript
// apps/storefront/src/hooks/useUser.ts
...
```typescript
// apps/storefront/src/routes/ProfilePage.tsx
...
pnpm --filter @hk26/storefront typecheck
...
<div>{user.invalidField}</div>

// ❌ TypeCheck error: Type 'User | undefined' cannot be used
<div>{user.fullName}</div> // Need to check if user exists first!
...
```typescript
// apps/storefront/src/hooks/useReviews.ts
...
```typescript
// apps/storefront/src/routes/ProductDetailPage.tsx
