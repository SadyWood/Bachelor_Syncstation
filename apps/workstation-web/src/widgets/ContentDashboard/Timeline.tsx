...
export function TimelineWidget({ title, onClose }: Omit<WidgetProps, 'id'>) {
  ...
  const allMarkers = React.useMemo(
    () => (SHOW_SCENE_CUT_MARKERS ? [...markers, ...sceneCutMarkers] : markers),
    [markers],
  );
  ...
  const timelineItems: HoolsyTimelineItem[] = React.useMemo(() => {
    ...
    const items = appearances.flatMap((appearance) => {
      ...
      const appearanceNumber =
        subjectAppearances.findIndex((a) => a.appearanceId === appearance.appearanceId) + 1;
  ...
  React.useEffect(() => {
    ...
    if (tracks.length === 0) {
      const activeTracks = DEFAULT_TIMELINE_TRACKS.filter((track) =>
        usedSubjectTypes.has(track.id),
      ).map((t) => ({
        id: t.id,
        label: t.label,
      }));
    ...
    const tracksToRemove = new Set([...currentTrackIds].filter((id) => !usedSubjectTypes.has(id)));
  ...
  const handleItemsChange = useCallback(
    (updatedItems: HoolsyTimelineItem[]) => {
      logger.warn(
        '[TIMELINE DEMO MODE] Timeline items modified. Changes are NOT saved to the server.',
      );
