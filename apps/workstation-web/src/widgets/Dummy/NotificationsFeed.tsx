    ...
    <BaseWidget title={title} onClose={onClose} titleIcon={Bell} {...props}>
      <div className="p-3 space-y-2">
        <div className="ws-pills">
          <button className="ws-pill" aria-selected="true">
            All
          </button>
        ...
        <div className="space-y-1 overflow-auto" style={{ maxHeight: 'calc(100% - 80px)' }}>
          {feed.map((m) => (
            <div
              key={`${m.time}-${m.text}`}
              className="ws-block p-2 flex gap-3 items-center ws-block-interactive"
            >
        ...
        <div className="ws-alert ws-alert-info">
          <div className="text-sm">Tips: trykk og dra for å omorganisere widgets.</div>
        </div>
      </div>
    </BaseWidget>
  );
