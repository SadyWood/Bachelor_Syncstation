...
const RegisterPage: React.FC = () => {
  ...
  const [invite, setInvite] = useState<{ email: string; platform?: string; isAvailable?: boolean; state?: 'available' | 'expired' | 'consumed' | 'revoked' } | null>(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setInvite({ email: info.email, platform: info.platform, isAvailable: info.isAvailable, state: info.state as 'available' | 'expired' | 'consumed' | 'revoked' });
          ...
          <div className="p-4 space-y-3">
            ...
            {invite && invite.isAvailable !== false && (
              <div className="ws-alert ws-alert-success">
                <div className="text-xs">Invited to <strong>{orgLabel}</strong></div>
              </div>
            )}
            {invite && invite.isAvailable === false && (
              <div className="ws-alert ws-alert-error text-xs">{error ?? 'Invite not available.'}</div>
            )}

            <div className="space-y-1">
              <label className="text-xs ws-muted">Full name</label>
              <input className="ws-input" placeholder="Jane Doe" value={fullName} onChange={e => setFullName(e.target.value)} required />
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] ws-muted">
                Invite-only: e-post kan ikke endres.
              </p>
            </div>
            ...
            <div className="space-y-1">
              ...
              <input className="ws-input" type="password" placeholder="••••••••" value={confirm} onChange={e => setConfirm(e.target.value)} required />
            </div>

            <label className="flex items-center gap-2 text-xs ws-muted pt-1">
              <input type="checkbox" className="rounded border-[var(--ws-border-light)]" checked={agree} onChange={e => setAgree(e.target.checked)} />
              I agree to the Terms & Privacy
            </label>

            <div className="pt-2">
              <Button appearance="solid" size="md" className="w-full" disabled={!invite || loading || invite.isAvailable === false}>
                Create account
              </Button>
            </div>
