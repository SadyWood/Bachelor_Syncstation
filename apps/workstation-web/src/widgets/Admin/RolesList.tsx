// apps/workstation-web/src/widgets/Admin/RolesList.tsx
...
const logger = createLogger('RolesList');
...
export default function RolesListWidget({ title, onClose, ...props }: WidgetProps) {
  ...
  // Fetch roles with useSWR - automatic caching and revalidation
  const {
    data: rows = [],
    error: swrError,
    isLoading,
        ...
        <div className="ws-table-wrap flex-1 ws-scroll-y">
          <table className="ws-table">
            ...
            <tbody>
              ...
              {!isLoading && filteredSorted.length > 0 && filteredSorted.map((r) => {
